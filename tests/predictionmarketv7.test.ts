...
describe("predictstacks (STX-native)", () => {
  ...
  it("cancel-market rejects non-admin caller", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    ...
    const create = simnet.callPublicFn(
      CONTRACT,
      "create-market",
      [Cl.stringAscii("cancel-ref"), Cl.uint(1000)],
      deployer
    );
    expect(cvToString(create.result)).toBe("(ok u1)");
  ...
  it("claim-winnings rejects before resolution", () => {
    ...
    const create = simnet.callPublicFn(
      CONTRACT,
      "create-market",
      [Cl.stringAscii("claim-ref"), Cl.uint(1000)],
      deployer
    );
    expect(cvToString(create.result)).toBe("(ok u1)");
    ...
    expect(cvToString(claim.result)).toBe("(err u103)");
  });

  it("refund-bets rejects when market not cancelled", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    ...
    const create = simnet.callPublicFn(
      CONTRACT,
      "create-market",
      [Cl.stringAscii("refund-ref"), Cl.uint(1000)],
      deployer
    );
    expect(cvToString(create.result)).toBe("(ok u1)");
    ...
    expect(cvToString(refund.result)).toBe("(err u114)");
  });

  it("pause blocks betting", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    ...
    const create = simnet.callPublicFn(
      CONTRACT,
      "create-market",
      [Cl.stringAscii("pause-ref"), Cl.uint(100)],
      deployer
    );
    expect(cvToString(create.result)).toBe("(ok u1)");
    ...
    expect(cvToString(bet.result)).toBe("(err u109)");
  });

  it("unpause re-enables betting", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    ...
    const create = simnet.callPublicFn(
      CONTRACT,
      "create-market",
      [Cl.stringAscii("unpause-ref"), Cl.uint(100)],
      deployer
    );
    expect(cvToString(create.result)).toBe("(ok u1)");
  ...
  it("rejects place-bet on a resolved market", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    ...
    simnet.callPublicFn(CONTRACT, "create-market", [Cl.stringAscii("resolved-bet-ref"), Cl.uint(10)], deployer);
    ...
    const bet = simnet.callPublicFn(
      CONTRACT, "place-bet",
      [Cl.uint(1), Cl.bool(false), Cl.uint(20_000)],
      wallet1
    );
    expect(cvToString(bet.result)).toBe("(err u102)");
  });

  it("rejects place-bet on a cancelled market", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    ...
    simnet.callPublicFn(CONTRACT, "create-market", [Cl.stringAscii("cancelled-bet-ref"), Cl.uint(100)], deployer);
    ...
    expect(cvToString(bet.result)).toContain("err");
  });

  it("rejects claim-winnings when no position was placed", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    const wallet2 = accounts.get("wallet_2")!;
    ...
    simnet.callPublicFn(CONTRACT, "create-market", [Cl.stringAscii("no-position-ref"), Cl.uint(10)], deployer);
    simnet.callPublicFn(CONTRACT, "place-bet", [Cl.uint(1), Cl.bool(true), Cl.uint(50_000)], wallet1);
    simnet.mineEmptyBlocks(20);
    simnet.callPublicFn(CONTRACT, "resolve-market", [Cl.uint(1), Cl.bool(true)], deployer);
  ...
  it("prevents double claim on the same market", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    const wallet2 = accounts.get("wallet_2")!;
    ...
    simnet.callPublicFn(CONTRACT, "create-market", [Cl.stringAscii("double-claim-ref"), Cl.uint(10)], deployer);
    simnet.callPublicFn(CONTRACT, "place-bet", [Cl.uint(1), Cl.bool(true), Cl.uint(50_000)], wallet1);
    simnet.callPublicFn(CONTRACT, "place-bet", [Cl.uint(1), Cl.bool(false), Cl.uint(20_000)], wallet2);
    simnet.mineEmptyBlocks(20);
    simnet.callPublicFn(CONTRACT, "resolve-market", [Cl.uint(1), Cl.bool(true)], deployer);

    const first = simnet.callPublicFn(CONTRACT, "claim-winnings", [Cl.uint(1)], wallet1);
    expect(cvToString(first.result)).not.toContain("err");
  ...
  it("rejects cancel-market when market is already resolved", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    ...
    simnet.callPublicFn(CONTRACT, "create-market", [Cl.stringAscii("cancel-resolved-ref"), Cl.uint(10)], deployer);
    simnet.callPublicFn(CONTRACT, "place-bet", [Cl.uint(1), Cl.bool(true), Cl.uint(50_000)], wallet1);
    simnet.mineEmptyBlocks(20);
    simnet.callPublicFn(CONTRACT, "resolve-market", [Cl.uint(1), Cl.bool(true)], deployer);
  ...
  it("rejects claim-refund when no position was placed on a cancelled market", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    const wallet2 = accounts.get("wallet_2")!;
    ...
    simnet.callPublicFn(CONTRACT, "create-market", [Cl.stringAscii("refund-nopos-ref"), Cl.uint(100)], deployer);
    simnet.callPublicFn(CONTRACT, "place-bet", [Cl.uint(1), Cl.bool(true), Cl.uint(50_000)], wallet1);
    simnet.callPublicFn(CONTRACT, "cancel-market", [Cl.uint(1)], deployer);
  ...
  it("NO outcome resolution — NO bettors can claim winnings", () => {
    const accounts = simnet.getAccounts();
    const deployer = accounts.get("deployer")!;
    const wallet1 = accounts.get("wallet_1")!;
    const wallet2 = accounts.get("wallet_2")!;
    ...
    simnet.callPublicFn(CONTRACT, "create-market", [Cl.stringAscii("no-outcome-win-ref"), Cl.uint(10)], deployer);
    // wallet1 bets YES, wallet2 bets NO
    ...
    // wallet2 (NO) should be able to claim
    const claim = simnet.callPublicFn(CONTRACT, "claim-winnings", [Cl.uint(1)], wallet2);
