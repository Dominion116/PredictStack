;; ============================================================================
;; PredictStack V7 - Peer-to-Peer Prediction Market Platform
;; ============================================================================
...
(define-data-var platform-admin-count uint u0)
...
(define-private (is-admin-address (address principal))
  (match (map-get? platform-admins { admin: address })
...
(define-public (initialize 
  ...
  (begin
    ...
    (map-set platform-admins { admin: admin } { enabled: true })
    (var-set platform-admin-count u1)
...
(define-public (set-admin (new-admin principal))
  (begin
    ...
    (let ((old-admin (var-get platform-admin)))
      ...
      (if (not (is-admin-address new-admin))
        (begin
          (map-set platform-admins { admin: new-admin } { enabled: true })
          (var-set platform-admin-count (+ (var-get platform-admin-count) u1))
        )
        true
      )
      (print {
...
;; Add admin address (admin only)
(define-public (add-admin (new-admin principal))
  (begin
    (asserts! (var-get contract-initialized) ERR-NOT-INITIALIZED)
    (asserts! (is-admin) ERR-NOT-AUTHORIZED)

    (if (is-admin-address new-admin)
      (ok true)
      (begin
        (map-set platform-admins { admin: new-admin } { enabled: true })
        (var-set platform-admin-count (+ (var-get platform-admin-count) u1))
...
;; Remove admin address (admin only)
(define-public (remove-admin (admin principal))
  (begin
    (asserts! (var-get contract-initialized) ERR-NOT-INITIALIZED)
    (asserts! (is-admin) ERR-NOT-AUTHORIZED)
    (asserts! (is-admin-address admin) ERR-NOT-AUTHORIZED)
    (asserts! (> (var-get platform-admin-count) u1) ERR-NOT-AUTHORIZED)
    (asserts! (not (is-eq admin (var-get platform-admin))) ERR-NOT-AUTHORIZED)

    (map-delete platform-admins { admin: admin })
    (var-set platform-admin-count (- (var-get platform-admin-count) u1))
    (print {
      event: "admin-removed",
      admin: admin,
...
(define-read-only (get-platform-config)
  (ok {
    ...
    admin-count: (var-get platform-admin-count),
